...
export default function Navbar({ activeTab, setActiveTab }) {
  ...
  return (
    <nav className="bg-white sticky top-0 z-50 px-6 py-4 flex items-center justify-between border-b border-slate-200 shadow-sm">
      ...
      <div className="relative flex items-center bg-slate-100/80 p-1 rounded-xl border border-slate-200/40 w-[380px] sm:w-[420px] shadow-inner">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative z-10 flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors duration-300 cursor-pointer ${
                isActive
                  ? 'text-white'
